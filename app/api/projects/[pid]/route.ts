...
export async function PUT(req: Request, {params}: { params: {pid: string} }) {
  try {
  ...
  console.log(pid)
    const data = await supabase.from('project').update({...project, dateupdated: new Date().toISOString()}).eq('id', Number(pid));
    console.log(data)
