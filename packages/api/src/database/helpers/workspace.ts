...
import { projectTableSetup } from './project.js';
import { teamTableSetup } from './team.js';
import { setupWorkspaceMembersAndRoles } from './member.js';
import { createIssueTables } from '../migrations/1716687527788.js';
...
export async function createWorkspaceTenant(db: Kysely<any>, input: WorkspaceCreateInput) {
  const workspace = await db.transaction().execute(async (trx) => {
    ...
    await trx
      .withSchema(`workspace_${workspace.id}`)
      .insertInto('member')
      .values({
        id: input.ownerId,
        email: user.email,
