...
export default function ProjectLayout({
  ...
  return (
     ...
     <div className="flex flex-row h-screen w-screen">
        <section id="sidebar" className="w-1/6 border-r border-gray-100 max-w-sm flex flex-col ">
            <Text  size="4" className="p-4 font-bold h-12">Orbit</Text>
            <div className="flex flex-col flex-grow overflow-y-auto p-2 border-gray-100 border-t gap-3">
            <CommandMenu setOpen={openSearch} open={search} />
                <Box className="w-full p-1 pb-3">

  <button className="bg-white w-full h-8 rounded-sm shadow-sm border border-gray-200 text-xs text-left p-1 px-2 text-gray-500" onClick={() => openSearch(true)}
                
                ><span className="flex justify-between items-center">Search <span className="bg-gray-100 px-1 rounded-sm border border-gray-200 shadow-sm text-[9px]">cmd + k</span></span></button>

</Box>
<section className="flex flex-col border-gray-100 ">
  <Link href={'/projects'} className=" flex w-full items-center h-8 text-sm  text-left p-1 px-2 text-gray-700">
    <BoxIcon className="h-3 w-3 " />
    <span className="pl-2 flex justify-between items-center h-full">projects</span>
  </Link>
            ...
            <div className="flex flex-col  overflow-y-auto p-2 gap-2">
              <Link href={'/settings'} className="bg-white flex w-full items-center h-8 rounded-sm shadow-sm border border-gray-200 text-xs text-left p-1 px-2 text-gray-500">
                <GearIcon className="h-3 w-3 mr-2" />
                <span>Settings</span>
              </Link>
