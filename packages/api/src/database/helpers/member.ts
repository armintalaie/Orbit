...
export async function memberTableSetup(trx: Transaction<any>, workspaceSchema: string) {
  await trx
    ...
    .addColumn('email', 'text', (col) =>
      col.notNull().unique().references('auth.users.email').onUpdate('cascade').onDelete('cascade').primaryKey()
    )
    ...
    .addColumn('id', 'uuid', (col) =>
      col.notNull().unique().references('auth.users.id').onUpdate('cascade').onDelete('cascade')
    )
    .execute();
...
async function createStarterRoles(trx: Transaction<any>, workspaceSchema: string) {
  ...
  await trx.withSchema(workspaceSchema).insertInto('role_permission').values(rolePermissionValues).execute();
}
