import { statusIconMapper } from '@/components/general/statusIconMapper';
...
export default function IssueStatusField({ statusId, issueId, contentOnly = false, reload }: IssueStatusFieldProps) {
  const { status: statusOptionsArr, fetcher } = useContext(OrbitContext);
  const statusOptions = groupById(statusOptionsArr);
  const doneStatusId = Object.keys(statusOptions).find((m) => statusOptions[Number(m)].label === 'Done');
  ...
  function groupById(array: IStatus[]): {
    [key: number]: IStatus;
  } {
    return array.reduce(
      (hash, obj) => ({
  ...
  async function updateStatus(id: number) {
    ...
    if (!isConfettiAnimating && doneStatusId && data.statusid.toString() === doneStatusId.toString()) {
      confettiReward();
    }
  }
  ...
  function filter(value: string, search: string) {
    try {
      ...
      return statusOptions[Number(value)].label.toLowerCase().indexOf(search.toLowerCase()) !== -1 ? 1 : 0;
    } catch (error) {
      return 0;
    }
  }
        ...
        <CommandList>
          ...
          <CommandGroup>
            {Object.keys(statusOptions).map((id) => (
              <CommandItem key={id} value={id.toString()} onSelect={onSelectedStatusChange}>
        ...
        <PopoverTrigger asChild>
          <Button variant='ghost' className='w-full justify-start p-0'>
            <div className='flex items-center space-x-2 text-2xs'>
              {statusIconMapper(statusOptions[selectedStatusId].label, 'h-4 w-4')}
