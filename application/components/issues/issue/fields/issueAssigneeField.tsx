...
export function IssueAssigneeField(props: AssigneeUpdateFieldProps) {
  ...
  const [selectedStatusId, setSelectedStatusId] = useState<string | null>(user ? user.id : null);
  ...
  const issueAssigneeSection = (
    <Command filter={filter}>
      ...
      <CommandList>
        ...
        <CommandGroup>
          <CommandItem key={-1} value={'unassigned'} onSelect={onSelectedUserChange}>
          ...
          {Object.entries(memberOptions).map(([, member]) => (
            <CommandItem key={member.id} value={member.id} onSelect={onSelectedUserChange}>
              <AssigneeAvatar
                assignee={member}
                formatting={{
                  showFullName: true,
  ...
  return (
    <div className='flex items-center space-x-4'>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className='overflow-hidden p-0' side='right' align='start'>
