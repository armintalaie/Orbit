...
export async function createIssueTables(trx: Transaction<any>, workspaceSchema: string) {
  await trx
    ...
    .schema.createTable('issue_status')
  ...
  await trx.withSchema(workspaceSchema).insertInto('issue_status').values(DEFAULT_ISSUE_STATUSES).execute();
  ...
  await trx
    ...
    .addColumn('status_id', 'integer', (col) =>
      col.references(`${workspaceSchema}.issue_status.id`).notNull().onUpdate('cascade').onDelete('cascade')
    )
    .addColumn('target_date', 'timestamptz')
    .addColumn('team_id', 'serial', (col) =>
      col.references('team.id').notNull().onUpdate('cascade').onDelete('cascade')
    )
  ...
  await trx
    .withSchema(workspaceSchema)
    .schema.createTable('issue_assignee')
    .addColumn('issue_id', 'serial', (col) =>
      col.references(`${workspaceSchema}.issue.id`).notNull().onUpdate('cascade').onDelete('cascade')
    )
    .addColumn('user_id', 'uuid', (col) =>
      col.references('member.id').notNull().onUpdate('cascade').onDelete('cascade')
    )
  ...
  await trx
    .withSchema(workspaceSchema)
    .schema.createTable('project_issue')
    .addColumn('project_id', 'serial', (col) =>
      col.references('project.id').notNull().onUpdate('cascade').onDelete('cascade')
    )
    .addColumn('issue_id', 'serial', (col) =>
      col.references(`${workspaceSchema}.issue.id`).notNull().onUpdate('cascade').onDelete('cascade')
    )
