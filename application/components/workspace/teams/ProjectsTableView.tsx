...
import { TableHead, Table, TableRow, TableHeader, TableCell, TableBody } from '../../ui/table';
import Link from 'next/link';
import { Badge } from '../../ui/badge';

export default function ProjectsTableView({ projects }: { projects: IProject[] }) {
  return (
          ...
          {projects.map((project: IProject) => (
            ...
            >
              <TableCell className='flex flex-row items-center gap-4'>
                <Link href={`/projects/${project.id}`} shallow={true} className='text-xs underline'>
                  <Maximize2 className='h-3 w-3' />
                </Link>
                {project.title}
              </TableCell>

              <TableCell className='hidden lg:table-cell'>{project.description}</TableCell>
