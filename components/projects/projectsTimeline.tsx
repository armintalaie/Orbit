...
export default function ProjectsTimelineView({ projects }) {
  ...
  return (
    <div className='flex w-full h-full rounded-lg  border border-gray-100 bg-white overflow-hidden'>
    <div onScroll={handleScroll} style={{ width: '100%', overflowX: 'auto' }}>
      <div style={{ display: 'flex', position: 'sticky', top: 0, zIndex: 2 }}>
        {dates.map((date, index) => {
          ...
          return (
            <div
              key={index}
              className=' text-sm flex min-w-[400px] flex-col items-center justify-center border-b border-gray-100 p-1 font-medium'
            >
      ...
      <div className='sticky z-10  flex'>
        {dates.map((date, index) => {
          ...
          return (
            <div
              key={index}
              className={`text-sm border-b border-gray-100 flex justify-center p-1 font-medium ${shouldDisplayMonth ? 'min-w-[50px] border-l border-gray-100' : 'min-w-[50px]'}`}
            >
      ...
      {projects.map((project, index) => (
        <div key={index} className='left sticky flex w-fit bg-gray-50  '>
          <Link className='h-12 sticky left-0 flex  w-[200px] items-center border-b border-gray-200 bg-white p-1 px-2 text-2xs line-clamp-1 hover:shadow-lg' href={`/projects/${project.id}`}>
            {project.title}
          </Link>
          {dates.map((date, index) => {
            ...
            const isOnStartDate = formatDate(utcDate) === formatDate(project.datestarted);
            const isOnDeadline = formatDate(utcDate) === formatDate(project.deadline);

            let className = 'h-12 min-w-[50px]  py-2 overflow-hidden flex justify-center items-center  ';
            ...
            switch (true) {
              case isOnStartDate:
                className2 += 'bg-gray-100 pl-4 rounded-l-xl border-l border-y  ';
                break;
              case isOnDeadline:
                className2 += ' bg-gray-100 pr-4 rounded-r-xl border-r border-y';
                break;
              case isWithinProjectDuration:
            ...
            return (<div key={index}   className={className}>
              ...
              <Link className={className2} href={`/projects/${project.id}`}/>
              {/* )} */}
