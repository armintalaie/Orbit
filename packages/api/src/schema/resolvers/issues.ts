...
export const issuesResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  _
...
export const projectIssuesResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  info
...
export const getIssues = async ({
  ...
  wid: string;
  db: Kysely<WorkspaceSchema>;
  pid?: string;
  tid?: string;
}) => {
  let query = db
    .withSchema(`workspace_${wid}`)
    .selectFrom('issue')
    .innerJoin('issue_status', 'issue_status.id', 'issue.status_id');
    ...
    .select((eb) => [
      jsonArrayFrom(
        eb
          ...
          .selectAll()
          .whereRef('issue_id', '=', 'issue.id')
          .innerJoin('member', 'member.id', 'issue_assignee.user_id')
      ).as('assignees'),
    ])
    .execute();
...
function getIssue({ wid, db, id }: { wid: string; db: Kysely<WorkspaceSchema>; id: number }): any {
  return db
    .withSchema(`workspace_${wid}`)
    .selectFrom('issue')
    .leftJoin('issue_status', 'issue_status.id', 'issue.status_id')
    .selectAll(['issue'])
    .where('issue.id', '=', id)
    .select((eb) => [
      jsonBuildObject({
    ...
    .select((eb) => [
      ...
      jsonArrayFrom(
        eb
          .selectFrom('project_issue')
          .selectAll()
          .whereRef('issue_id', '=', 'issue.id')
          .innerJoin('project', 'project.id', 'project_issue.project_id')
      ).as('projects'),
    ])
    .executeTakeFirstOrThrow();
}

export const issueResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  _
...
export const updateIssueResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  _
  ...
  if (issueInput['targetDate'] !== undefined) {
    issueInput['targetDate'] =
      issueInput['targetDate'] == null ? null : new Date(Number(issueInput['targetDate'])).toISOString();
  }
      ...
      if (assignees) {
        ...
        if (assignees.length > 0) {
          await trx
            ...
            .values(assignees.map((assignee: any) => ({ issue_id: id, user_id: assignee.id })))
            .execute();
        }
      }
      if (projects) {
        ...
        if (projects.length > 0) {
          await trx
            .insertInto('project_issue')
            .values(projects.map((project: any) => ({ issue_id: id, project_id: project.id })))
            .execute();
        }
      }
...
export const createIssueResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  _
    ...
    return d2
      ...
      .execute(async (trx) => {
        ...
        if (assignees && assignees.length > 0) {
          await trx
            .insertInto('issue_assignee')
            .values(assignees.map((assignee: any) => ({ issue_id: newIssue.id, user_id: assignee.id })))
            .execute();
        }
        if (projects && projects.length > 0) {
          await trx
            .insertInto('project_issue')
            .values(projects.map((project: any) => ({ issue_id: newIssue.id, project_id: project })))
            .execute();
        }
 ...
 */
export const deleteIssueResolver: GraphQLFieldResolver<any, { db: Kysely<WorkspaceSchema> }> = async (
  parent: any,
  args: any,
  context,
  _
