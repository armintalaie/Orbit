...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db
    ...
    .addColumn('workspace_id', 'uuid', (col) => col.references('workspace.id').notNull().onUpdate('cascade').onDelete('cascade'))
    .addColumn('user_id', 'uuid', (col) => col.references('auth.users.id').notNull().onUpdate('cascade').onDelete('cascade'))
